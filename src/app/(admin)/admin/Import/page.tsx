  ...
  return (
    <div className="p-4 sm:p-8 space-y-6 max-w-7xl mx-auto font-sans text-slate-100">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 bg-slate-900/80 border border-slate-800 rounded-2xl p-6 backdrop-blur-xl">
        ...
        {isParsed && (
          <button
            ...
            className="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl text-xs transition-all shadow-md flex items-center gap-2 cursor-pointer active:scale-95 font-mono"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Raw Input Column */}
        <div className="lg:col-span-5 bg-slate-900/80 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl">
          <div className="flex justify-between items-center border-b border-slate-800 pb-3">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider font-mono flex items-center gap-2">
              <FileSpreadsheet className="w-4 h-4 text-sky-400" /> Paste CSV or Directory Data
            </h3>
            <button
              onClick={handleClear}
          ...
          <div className="space-y-3">
            <textarea
              ...
              className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-slate-200 font-mono focus:outline-none focus:border-sky-500 leading-relaxed"
            />
        ...
        <div className="lg:col-span-7 bg-slate-900/80 border border-slate-800 rounded-2xl p-6 space-y-4 shadow-xl">
          <div className="flex justify-between items-center border-b border-slate-800 pb-3">
            <h3 className="text-xs font-bold text-white uppercase tracking-wider font-mono flex items-center gap-2">
              <Target className="w-4 h-4 text-emerald-400" /> Normalized Audit Links ({parsedLeads.length})
            </h3>
          ...
          <div className="space-y-3 max-h-[420px] overflow-y-auto pr-1">
            ...
            {parsedLeads.map((lead) => (
              <div
                key={lead.id}
                className="bg-slate-950 border border-slate-800/80 rounded-xl p-4 space-y-2 text-xs font-sans hover:border-slate-700 transition-colors"
              >
                <div className="flex justify-between items-start">
                  <div>
                    <h4 className="font-bold text-white text-sm">{lead.businessName}</h4>
                    <span className="text-[10px] text-slate-400 font-mono">{lead.city} • {lead.phone}</span>
                  </div>
                  <span className="text-[9px] font-mono px-2 py-0.5 bg-sky-500/10 border border-sky-500/20 text-sky-400 rounded-md font-bold">
                    {lead.trade}
                  </span>
                </div>

                {/* Audit Link Preview */}
                <div className="p-2.5 bg-slate-900/80 border border-slate-800 rounded-lg flex justify-between items-center text-[10px] font-mono">
                  ...
                  <button
                    ...
                    className="text-slate-400 hover:text-white flex items-center gap-1 font-bold shrink-0"
                  >
