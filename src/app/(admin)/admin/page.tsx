...
function KpiCardSkeleton() {
  return (
    <div className="bg-slate-950/80 border border-slate-800 p-3 rounded-2xl flex flex-col justify-between animate-pulse">
      ...
      {isOpen && (
        ...
        >
          <motion.div
            ...
            className="bg-slate-900 border border-slate-800 p-4 rounded-3xl shadow-2xl max-w-md w-full space-y-4"
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              ...
              <button onClick={onClose} className="text-slate-400 hover:text-white p-1 rounded-lg transition-all">
                <X className="w-4 h-4" />
              </button>
            </div>
            ...
            <div className="space-y-2">
              {shortcuts.map((sc, i) => (
                <div key={i} className="flex justify-between items-center bg-slate-950 p-2.5 rounded-xl border border-slate-800/80">
                  <span className="text-slate-300 font-sans">{sc.description}</span>
                  <kbd className="bg-slate-900 text-indigo-300 border border-indigo-500/30 px-2 py-1 rounded-md text-xs font-bold">
                    {sc.key}
                  </kbd>
                </div>
              ))}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed top-0 right-0 h-full w-full max-w-md bg-slate-950 border-l border-slate-800 z-[150] p-4 shadow-2xl font-mono text-xs flex flex-col space-y-4 pt-[calc(max(16px,env(safe-area-inset-top))+16px)] pb-[calc(max(16px,env(safe-area-inset-bottom))+16px)]"
          >
            ...
            <div className="flex-1 overflow-y-auto space-y-2.5 pr-1">
              {filteredActivities.map((act) => (
                <div
                  ...
                  className="bg-slate-900/80 border border-slate-800/80 p-3.5 rounded-2xl space-y-1.5 hover:border-indigo-500/40 transition-all cursor-pointer group"
                >
                  <div className="flex justify-between items-center text-xs">
                    <span className="text-slate-400 font-bold">[{act.timestamp}]</span>
                    <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full font-bold uppercase border ${
                      ...
                      {act.type === 'booking' && <Calendar className="w-3 h-3 text-indigo-400" />}
                      {act.type === 'alert' && <AlertTriangle className="w-3 h-3 text-rose-400" />}
                      {act.type}
                    </span>
                  </div>
                  <div className="font-sans text-xs text-white font-bold flex items-center justify-between">
...
function FloatingSmartDock({
  ...
  return (
    <div className="fixed bottom-[max(12px,env(safe-area-inset-bottom))] left-1/2 -translate-x-1/2 z-[130] no-print px-2 w-full max-w-lg flex justify-center">
      <div className="bg-slate-900/95 border border-slate-800 backdrop-blur-xl px-3 py-2 rounded-full shadow-2xl flex items-center justify-between gap-1.5 font-mono text-xs w-full max-w-md overflow-x-auto no-scrollbar">
        {activeWorkspaceLead ? (
          <>
            <button
            ...
            <button
              ...
              className="px-2.5 py-1.5 bg-indigo-600/30 text-indigo-200 border border-indigo-500/40 rounded-full font-bold text-xs uppercase hover:bg-indigo-600 shrink-0 cursor-pointer flex items-center gap-1"
            >
            ...
            <button
              ...
              className="px-3.5 py-1.5 bg-gradient-to-r from-emerald-500 to-indigo-600 text-white font-black rounded-full text-xs uppercase shadow-lg shrink-0 cursor-pointer"
            >
              🧮 Quoter
            </button>
          ...
          <>
            <button
              ...
              className="w-full py-2.5 px-4 bg-gradient-to-r from-emerald-600 via-teal-600 to-indigo-600 hover:from-emerald-500 hover:to-indigo-500 text-white font-black text-xs uppercase font-mono rounded-full shadow-lg transition-all cursor-pointer active:scale-95 flex items-center justify-center gap-2 border border-slate-700 min-h-[40px]"
            >
            ...
            <button
              ...
              className="px-3 py-2 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-200 rounded-full font-bold text-xs cursor-pointer transition-all flex items-center justify-center gap-1 shrink-0 min-h-[40px]"
            >
              <Search className="w-3.5 h-3.5 text-slate-400" />
              <span className="text-sm bg-slate-900 px-1.5 py-0.5 rounded text-indigo-300 font-mono font-bold">Search</span>
            </button>
          </>
        )}
      </div>
...
function CommandPaletteModal({
  ...
  return (
    ...
    >
      <div
        className="bg-slate-900 border border-slate-800 max-w-lg w-full rounded-2xl shadow-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-3 border-b border-slate-800 flex items-center gap-2">
          ...
          />
          <span className="text-sm bg-slate-800 px-1.5 py-0.5 rounded text-slate-400 font-bold">ESC</span>
        </div>
        ...
        <div className="max-h-60 overflow-y-auto p-2 space-y-1">
          <div
            ...
            className="p-2.5 hover:bg-indigo-600/20 rounded-xl cursor-pointer flex justify-between items-center text-slate-200"
          >
            <span>🧮 Launch Interactive Quoter</span>
            <span className="text-sm text-indigo-400 font-bold">Alt+Q</span>
          </div>
          {leads
            ...
            .map((lead) => (
              <div
                ...
                className="p-2.5 hover:bg-emerald-600/20 rounded-xl cursor-pointer flex justify-between items-center text-slate-200"
              >
...
export default function AdminPage() {
  ...
  return (
    <div ref={pageRef} className={`admin-console min-h-[100dvh] antialiased relative pb-36 select-none transition-colors pt-[calc(max(12px,env(safe-area-inset-top))+8px)] ${
      ...
      <div className="fixed top-1/4 left-1/4 w-96 h-96 bg-indigo-600/10 rounded-full blur-3xl pointer-events-none -z-10" />
      <div className="fixed bottom-1/4 right-1/4 w-96 h-96 bg-purple-600/10 rounded-full blur-3xl pointer-events-none -z-10" />
      ...
      <header className="max-w-7xl mx-auto px-3 sm:px-4 pt-2 sm:pt-4 no-print relative z-10 font-mono text-xs">
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl sm:rounded-3xl p-3 sm:p-4 backdrop-blur-2xl shadow-2xl space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 sm:gap-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 w-full lg:w-auto">
              {isLoadingLeads ? (
                ...
                <>
                  <div className="gsap-card bg-slate-950/80 border border-slate-800 hover:border-slate-700 transition-colors p-3 rounded-2xl flex flex-col justify-between">
                  ...
                  <div className="gsap-card bg-slate-950/80 border border-slate-800 hover:border-slate-700 transition-colors p-3 rounded-2xl flex flex-col justify-between">
                    ...
                    <div className="mt-1">
                      <div className="text-base font-bold text-white font-mono">{leads.length || 14}</div>
                      <div className="flex items-center text-sm text-emerald-400 font-medium mt-0.5">
                  ...
                  <div className="gsap-card bg-slate-950/80 border border-slate-800 hover:border-slate-700 transition-colors p-3 rounded-2xl flex flex-col justify-between">
                    ...
                    <div className="mt-1">
                      <div className="text-base font-bold text-white font-mono">1.8m</div>
            ...
            <div className="flex flex-wrap items-center gap-2 w-full lg:w-auto justify-end">
              <button
                ...
                className="px-3 py-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-emerald-400 font-semibold rounded-xl cursor-pointer transition-all text-xs flex items-center gap-1.5 shrink-0"
              >
              ...
              <button
                ...
                className="px-3 py-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-amber-300 font-semibold rounded-xl cursor-pointer transition-all text-xs flex items-center gap-1.5 shrink-0"
              >
              ...
              <button
                ...
                className={`px-3 py-2.5 border rounded-xl cursor-pointer transition-all text-xs font-semibold flex items-center gap-1.5 shrink-0 ${
                  showOpsDrawer ? 'bg-indigo-600 border-indigo-500 text-white' : 'bg-slate-950 border-slate-800 text-slate-300 hover:bg-slate-800'
                }`}
              >
                Ops {showOpsDrawer ? '▲' : '▼'}
              </button>
              ...
              <button
                ...
                className="p-2.5 bg-slate-950 hover:bg-slate-800 border border-slate-800 text-slate-300 rounded-xl cursor-pointer transition-all shrink-0"
              ...
              <button
                ...
                className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-xl shadow-lg transition-all cursor-pointer flex items-center gap-1.5 shrink-0"
              >
                Demo Portal
              </button>
            </div>
            ...
            <div className="space-y-3 pt-3 border-t border-slate-800 animate-fadeIn">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div className="bg-slate-950 border border-slate-800 p-3 rounded-2xl flex justify-between items-center">
                  <span className="text-slate-400">Twilio Voice & SMS Trunk:</span>
                  <span className="text-emerald-400 font-bold">🟢 Healthy ($24.10)</span>
                </div>
                <div className="bg-slate-950 border border-slate-800 p-3 rounded-2xl flex justify-between items-center">
                  <span className="text-slate-400">AI Dispatch Latency:</span>
                  <span className="text-indigo-400 font-bold">⚡ 82ms</span>
                </div>
              </div>

              <div className="bg-slate-950 border border-slate-800 p-3 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-emerald-400 font-bold uppercase">🏢 Multi-Tenant Impersonation:</span>
                  <span className="text-slate-400 text-xs">Switch workspace view to test portal context.</span>
                </div>
                <div className="flex items-center gap-2">
                  <button onClick={() => handleLaunchDemoPortal('demo-apex-plumbing')} className="px-3 py-1 bg-slate-900 border border-slate-800 text-indigo-300 hover:text-white rounded-xl text-xs font-bold cursor-pointer">Apex Plumbing ➔</button>
                  <button onClick={() => handleLaunchDemoPortal('demo-summit-roofing')} className="px-3 py-1 bg-slate-900 border border-slate-800 text-indigo-300 hover:text-white rounded-xl text-xs font-bold cursor-pointer">Summit Roofing ➔</button>
                </div>
              </div>

              <div className="bg-slate-950 border border-slate-800 p-3 rounded-2xl flex items-center justify-between text-slate-300">
                <div className="flex items-center gap-3 overflow-x-auto no-scrollbar">
                  ...
                  {activityFeed.map((act) => (
                    <span key={act.id} className="shrink-0 bg-slate-900 border border-slate-800 px-2.5 py-1 rounded-xl text-slate-300 flex items-center gap-1.5">
                      <span className="text-slate-500">[{act.timestamp}]</span>
                      <strong className="text-white font-bold">{act.clientName}:</strong>
          ...
          <div className="flex flex-wrap justify-between items-center pt-3 border-t border-slate-800 gap-3">
            <div className="flex bg-slate-950 border border-slate-800 p-1 rounded-2xl shadow-xl w-full sm:w-auto justify-between sm:justify-start text-xs">
              <button
              ...
              >
                <Layers className="w-3.5 h-3.5" /> Pipeline <kbd className="hidden lg:inline text-[10px] opacity-60 normal-case">1</kbd>
              </button>
              <button
              ...
              >
                <Play className="w-3.5 h-3.5" /> Studio <kbd className="hidden lg:inline text-[10px] opacity-60 normal-case">2</kbd>
              </button>
              <button
              ...
              >
                <Bot className="w-3.5 h-3.5" /> Vault <kbd className="hidden lg:inline text-[10px] opacity-60 normal-case">3</kbd>
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-end flex-wrap">
              ...
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-950 border border-slate-800 text-slate-300 rounded-xl text-xs font-semibold cursor-pointer transition-all flex items-center gap-1 hover:text-white"
                title="Keyboard Shortcuts (Shift + ?)"
              >
                <Keyboard className="w-3 h-3 text-indigo-400" /> <span className="hidden sm:inline">Shortcuts</span>
              </button>
                ...
                {(['cyber', 'pro_blue', 'oled'] as BrandTheme[]).map((thm) => (
                  <button
                    ...
                    className={`px-2 py-1 rounded-lg text-[10px] font-bold uppercase transition-all ${
                      activeThemeMode === thm ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                    }`}
                  >
              ...
              <button
                onClick={() => setIsCommandPaletteOpen(true)}
                className="hidden sm:inline-flex items-center gap-1.5 text-xs bg-slate-950 border border-slate-800 text-indigo-300 px-3 py-2 rounded-xl font-semibold hover:bg-slate-800 transition-all cursor-pointer"
              >
      ...
      {(pinnedLeads.length > 0 || recentWorkspaceLeads.length > 0) && !activeWorkspaceLead && (
        <div className="max-w-7xl mx-auto px-4 mt-3 z-10 relative">
          <div className="bg-slate-900/60 border border-slate-800 rounded-2xl px-3 py-2 flex items-center gap-2 overflow-x-auto no-scrollbar font-mono text-xs">
              ...
              <>
                ...
                {recentWorkspaceLeads.map((rLead) => (
                  <button
                    ...
                    className="px-3 py-1 bg-slate-950 border border-slate-800 hover:bg-slate-800 text-slate-300 rounded-xl shrink-0 font-semibold transition-all cursor-pointer"
                  >
                    {rLead.businessName}
                  </button>
                ))}
      ...
      {isFormOpen && (
        <div className="max-w-7xl mx-auto px-4 mt-4 relative z-20 animate-fadeIn font-mono text-xs">
          <div className="bg-slate-900 border border-slate-800 p-3 rounded-3xl backdrop-blur-2xl shadow-2xl space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <h3 className="text-sm font-bold text-white uppercase flex items-center gap-2">
                <Plus className="w-4 h-4 text-emerald-400" /> Onboard New Client Account Node
              </h3>
              <button onClick={() => setIsFormOpen(false)} className="text-slate-400 hover:text-white font-bold cursor-pointer">✕ Close</button>
            </div>

            <form onSubmit={handleCreateNewClient} className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
              <div className="space-y-1">
                <label className="text-slate-400 font-bold text-xs">Business Name *</label>
                <input
                  type="text"
                  required
                  ...
                  className="w-full bg-slate-950 border border-slate-800 p-2.5 text-white rounded-xl outline-none focus:border-indigo-500 text-xs font-sans"
                />
              </div>

              <div className="space-y-1">
                <label className="text-slate-400 font-bold text-xs">Website Domain *</label>
                <input
                  type="text"
                  required
              ...
              <div className="space-y-1">
                <label className="text-slate-400 font-bold text-xs">Contact Email</label>
                <input
                  type="email"
              ...
              <div className="space-y-1">
                <label className="text-slate-400 font-bold text-xs">Phone Number</label>
                <input
                  type="tel"
              ...
              <div className="sm:col-span-2 md:col-span-4 flex justify-end gap-2 pt-2 border-t border-slate-800">
                <button
                  type="button"
                  onClick={() => setIsFormOpen(false)}
                  className="px-4 py-2 bg-slate-950 border border-slate-800 text-slate-300 rounded-xl font-bold cursor-pointer hover:bg-slate-800"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-6 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl cursor-pointer shadow-lg"
                >
        ...
        >
          ...
          {activeTab === 'pipeline' && (
            <div className="space-y-4 mt-4">
              ...
              <div className="px-4 max-w-7xl mx-auto relative z-10 no-print">
                <div className="bg-slate-900 border border-slate-800 p-3.5 rounded-2xl space-y-2 font-mono text-xs">
                  <div className="flex justify-between items-center">
                    <span className="text-indigo-300 font-bold uppercase text-xs flex items-center gap-1">
                      <Bot className="w-3.5 h-3.5 text-indigo-400" /> Autonomous Deal Rescue Agent
                    </span>
                    <span className="text-emerald-400 text-xs">Scan Active</span>
                  </div>
                  
                  {rescuingLeadId && generatedRescuePlay ? (
                    <div className="bg-slate-950 border border-indigo-500/30 p-3 rounded-xl space-y-2 animate-fadeIn">
                      <p className="text-slate-200 text-xs font-sans">{generatedRescuePlay}</p>
                      <div className="flex gap-2">
                        <button onClick={() => { playAudioChime('win'); navigator.clipboard.writeText(generatedRescuePlay); toast.success("🚀 Rescue Script Copied!"); }} className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-lg uppercase text-sm cursor-pointer">Approve & Copy</button>
                        <button onClick={() => setRescuingLeadId(null)} className="px-3 py-1.5 bg-slate-800 text-slate-400 rounded-lg font-bold uppercase text-sm cursor-pointer">Dismiss</button>
                      </div>
                    </div>
                  ) : (
                    <p className="text-slate-400 text-xs">Click "🤖 Rescue" on any stalled proposal in your pipeline below.</p>
                  )}
                </div>
              </div>
              ...
              <div className="px-4 max-w-7xl mx-auto relative z-10">
                ...
                <div className="mb-4 no-print bg-slate-900/60 border border-slate-800 rounded-2xl p-3 space-y-3 font-mono text-xs">
                  <div className="flex flex-wrap items-center justify-between gap-3">
                    <div className="flex gap-2 items-center w-full sm:w-auto flex-wrap">
                      <button onClick={() => setIsFormOpen(!isFormOpen)} className="flex-1 sm:flex-none px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-xl transition-all cursor-pointer flex items-center gap-1.5">
                        {isFormOpen ? <X className="w-3.5 h-3.5" /> : <Plus className="w-3.5 h-3.5" />}
                        {isFormOpen ? "Close" : "New Account"}
                      </button>

                      <div className="flex bg-slate-950 border border-slate-800 p-1 rounded-xl">
                        <button onClick={() => setViewMode('table')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer transition-all ${viewMode === 'table' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Table</button>
                        <button onClick={() => setViewMode('kanban')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer transition-all ${viewMode === 'kanban' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Kanban</button>
                        <button onClick={() => setViewMode('trello')} className={`px-3 py-1.5 rounded-lg text-xs font-semibold cursor-pointer transition-all ${viewMode === 'trello' ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'}`}>Trello</button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <div className="relative w-full sm:w-56">
                        ...
                        <input
                          ...
                          className="bg-slate-950 border border-slate-800 rounded-xl pl-9 pr-3 py-2 text-white placeholder-slate-500 text-xs w-full outline-none focus:border-indigo-500"
                        />
                      </div>
                      ...
                      <select
                        ...
                        className="bg-slate-950 border border-slate-800 rounded-xl px-3 py-2 text-white text-xs font-semibold outline-none cursor-pointer"
                      >
                  ...
                  <div className="flex flex-wrap items-center gap-2 pt-3 border-t border-slate-800">
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                        activePresetFilter === 'ALL' ? 'bg-indigo-600 text-white' : 'bg-slate-950 text-slate-400 border border-slate-800 hover:text-white'
                      }`}
                    >
                      All Decks
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg font-semibold transition-all flex items-center gap-1 ${
                        activePresetFilter === 'HIGH_LOSS' ? 'bg-rose-600 text-white' : 'bg-slate-950 text-rose-400 border border-rose-500/30 hover:bg-rose-950/40'
                      }`}
                    >
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                        activePresetFilter === 'STALLED' ? 'bg-amber-600 text-white' : 'bg-slate-950 text-amber-400 border border-amber-500/30 hover:bg-amber-950/40'
                      }`}
                    >
                      ⏳ Stalled Deals
                    </button>
                    <button
                      ...
                      className={`px-3 py-1 rounded-lg font-semibold transition-all ${
                        activePresetFilter === 'CLOSED_WON' ? 'bg-emerald-600 text-white' : 'bg-slate-950 text-emerald-400 border border-emerald-500/30 hover:bg-emerald-950/40'
                      }`}
                    >
                      🏆 Closed Won
                    </button>
                ...
                {viewMode === 'table' ? (
                  <div className="border border-slate-800 rounded-2xl shadow-2xl overflow-hidden bg-slate-900/60 backdrop-blur-xl font-mono text-xs">
                    <div className="p-4 border-b border-slate-800 flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        <button onClick={handleSelectAllLeads} className="text-slate-400 hover:text-white">
                        ...
                        <h2 className="text-sm font-black text-white uppercase">Pipeline Nodes</h2>
                      </div>
                      <span className="text-xs text-slate-400">{filteredLeads.length} Accounts Found</span>
                    </div>
                    ...
                    <div className="hidden md:block p-4 overflow-x-auto">
                      {isLoadingLeads ? (
                        ...
                        <div className="flex flex-col items-center justify-center gap-2 py-12 text-center">
                          ...
                          <p className="text-sm font-semibold text-slate-300">No accounts match your filters</p>
                        ...
                        <table className="w-full text-left text-xs text-slate-300 min-w-[650px]">
                          <thead className="text-xs uppercase font-bold bg-slate-950 text-slate-400 border-b border-slate-800">
                            <tr>
                              <th className="p-3 w-8">Select</th>
                              <th onClick={() => handleSortColumn('businessName')} className="p-3 cursor-pointer hover:text-white transition-colors select-none">
                                <div className="flex items-center gap-1">
                                  <span>Entity Identity</span>
                                  {sortField === 'businessName' ? (sortOrder === 'asc' ? <ArrowUp className="w-3 h-3 text-indigo-400" /> : <ArrowDown className="w-3 h-3 text-indigo-400" />) : <ArrowUpDown className="w-3 h-3 opacity-40" />}
                                </div>
                              </th>
                              <th onClick={() => handleSortColumn('overallScore')} className="p-3 cursor-pointer hover:text-white transition-colors select-none">
                                <div className="flex items-center gap-1">
                                  <span>Health Score</span>
                                  {sortField === 'overallScore' ? (sortOrder === 'asc' ? <ArrowUp className="w-3 h-3 text-indigo-400" /> : <ArrowDown className="w-3 h-3 text-indigo-400" />) : <ArrowUpDown className="w-3 h-3 opacity-40" />}
                                </div>
                              </th>
                              <th onClick={() => handleSortColumn('stage')} className="p-3 cursor-pointer hover:text-white transition-colors select-none">
                                <div className="flex items-center gap-1">
                                  <span>Stage</span>
                                  {sortField === 'stage' ? (sortOrder === 'asc' ? <ArrowUp className="w-3 h-3 text-indigo-400" /> : <ArrowDown className="w-3 h-3 text-indigo-400" />) : <ArrowUpDown className="w-3 h-3 opacity-40" />}
                                </div>
                              </th>
                          ...
                          <tbody className="divide-y divide-slate-800/80">
                            {filteredLeads.map((lead) => lead.id && (
                              <tr key={lead.id} className="gsap-card group hover:bg-slate-800/80 transition-all relative">
                                <td className="p-3">
                                  <button onClick={(e) => toggleSelectLead(lead.id, e)} className="text-slate-400 hover:text-white cursor-pointer">
                                ...
                                <td
                                  ...
                                  className="p-3 font-sans font-semibold text-white cursor-pointer relative"
                                >
                                  <div className="flex items-center gap-2">
                                    ...
                                    <SlidersHorizontal className="w-3 h-3 text-indigo-400 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
                                  </div>
                                  <span className="text-xs font-mono text-slate-400 block font-normal">{lead.url}</span>

                                  {/* Quick-Peek Popover */}
                                  {activePopoverLeadId === lead.id && (
                                    <div className="absolute left-4 top-12 z-50 bg-slate-900 border border-slate-800 p-3 rounded-2xl shadow-2xl space-y-2 text-xs font-mono animate-fadeIn w-64" onClick={(e) => e.stopPropagation()}>
                                      <div className="border-b border-slate-800 pb-1 flex justify-between items-center">
                                        <span className="font-bold text-white">{lead.businessName}</span>
                                      ...
                                      <div className="pt-1 flex gap-1">
                                        <button onClick={() => { openWorkspaceMatrix(lead); setActivePopoverLeadId(null); }} className="w-full text-center py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-lg font-bold text-xs cursor-pointer">Open Pitch Deck ➔</button>
                                      </div>
                                    </div>
                                  )}
                                </td>
                                ...
                                <td className="p-3">
                                  <select
                                    ...
                                    className="px-2.5 py-1.5 bg-slate-950 text-white rounded-lg text-xs font-semibold border border-slate-800 outline-none cursor-pointer hover:border-slate-700"
                                  >
                                    <option value="New Lead">New Lead</option>
                                    <option value="Pitched">Pitched</option>
                                    <option value="Proposal Sent">Proposal Sent</option>
                                ...
                                <td className="p-3 text-right">
                                  <div className="flex items-center justify-end gap-1.5">
                                    <button onClick={() => handleTriggerDealRescueAgent(lead)} className="px-2.5 py-1.5 bg-indigo-950 border border-indigo-500/30 text-indigo-300 rounded-lg text-xs font-semibold cursor-pointer hover:bg-indigo-900 transition-all">
                                      Rescue
                                    </button>
                                    <button onClick={() => openWorkspaceMatrix(lead)} className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold text-xs rounded-lg shadow-md transition-all cursor-pointer inline-flex items-center gap-1">
                      ...
                      {filteredLeads.length === 0 ? (
                        ...
                        <div key={lead.id} className="gsap-card pt-3 first:pt-0 space-y-3 font-sans">
                          <div className="flex items-start justify-between gap-2">
                            <div className="flex items-start gap-2 min-w-0">
                              ...
                              <div className="min-w-0">
                                ...
                                <p className="text-xs text-slate-400 font-mono mt-0.5 truncate">{lead.url}</p>
                              </div>
                            </div>
                          ...
                          <select
                            value={lead.stage || 'New Lead'}
                            onChange={(e) => handleUpdateStage(lead.id, e.target.value as DealStage)}
                            className="w-full px-2.5 py-2 bg-slate-950 text-white rounded-xl text-xs font-semibold border border-slate-800 outline-none cursor-pointer"
                          >
                            <option value="New Lead">New Lead</option>
                            <option value="Pitched">Pitched</option>
                            <option value="Proposal Sent">Proposal Sent</option>
                          ...
                          <div className="flex items-center space-x-2">
                            <button onClick={() => handleTriggerDealRescueAgent(lead)} className="flex-1 py-2 bg-indigo-950/80 border border-indigo-500/30 text-indigo-300 rounded-xl text-xs font-semibold">
                              Rescue
                            </button>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 font-mono text-xs">
                    {ALL_STAGES.map((stg) => {
                      ...
                      return (
                        <div key={stg} onDragOver={handleDragOver} onDrop={(e) => handleDropOnStage(e, stg)} className="bg-slate-900/80 border border-slate-800 rounded-2xl p-3 flex flex-col space-y-2 min-h-[300px] backdrop-blur-xl">
                          <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                            <span className="font-semibold text-slate-200 text-xs uppercase tracking-wide">{stg}</span>
                            <span className="bg-slate-800 text-slate-300 text-xs px-2 py-0.5 rounded-full font-bold">{stageLeads.length}</span>
                          </div>
                          <div className="space-y-2 flex-1">
                            {stageLeads.length === 0 ? (
                              <div className="text-xs text-slate-600 text-center py-6 border border-dashed border-slate-800 rounded-xl">Drop a deal here</div>
                            ) : stageLeads.map((lead) => (
                              <div key={lead.id} draggable onDragStart={(e) => handleDragStart(e, lead.id)} className="gsap-card bg-slate-950 border border-slate-800 p-3 rounded-xl space-y-1.5 cursor-grab shadow-md hover:border-slate-700 hover:bg-slate-800/80 transition-all">
                                <span className="font-bold text-white font-sans text-xs block truncate">{lead.businessName}</span>
                                <span className="text-xs text-slate-400 block truncate font-mono">{lead.url}</span>
                                <button onClick={() => openWorkspaceMatrix(lead)} className="text-xs bg-indigo-600 hover:bg-indigo-500 text-white px-3 py-1.5 rounded-lg font-semibold w-full transition-all cursor-pointer flex items-center justify-center gap-1">Open Deck <ArrowUpRight className="w-3 h-3" /></button>
                              </div>
                            ))}
                          </div>
                        </div>
          ...
          {activeTab === 'vault' && (
            <div className="max-w-7xl mx-auto p-4 font-mono text-xs space-y-6 mt-4">
              <div className="bg-slate-900/80 border border-slate-800 p-4 rounded-3xl backdrop-blur-xl flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                <div>
                  <h3 className="text-sm font-black text-white uppercase flex items-center gap-2">
                    <Bot className="w-4 h-4 text-purple-400" /> Sales Vault & Battle Cards
                  </h3>
                  <p className="text-slate-400 text-xs font-sans">Pick an industry sector to load field-tested objection counter-pitches.</p>
                </div>

                <div className="flex bg-slate-950 border border-slate-800 p-1 rounded-2xl flex-wrap">
                  {(['Home Services', 'Legal & Law', 'Dental & Medical', 'B2B & SaaS'] as Industry[]).map((ind) => (
                    <button
                      key={ind}
                      onClick={() => setActiveIndustry(ind)}
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold cursor-pointer transition-all ${
                        activeIndustry === ind ? 'bg-indigo-600 text-white' : 'text-slate-400 hover:text-white'
                      }`}
                    >
                      {ind}
                    </button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {(customBattleCards[activeIndustry] || []).map((card, idx) => (
                  <div key={idx} className="bg-slate-900/80 border border-slate-800 p-4 rounded-2xl space-y-2 backdrop-blur-xl">
                    <span className="text-amber-400 font-bold text-xs block">{card.objection}</span>
                    <p className="text-slate-300 text-xs font-sans leading-relaxed">{card.response}</p>
                    <button
                      onClick={() => {
                        navigator.clipboard.writeText(card.response);
                        toast.success("Copied battle card response!");
                      }}
                      className="px-3 py-1 bg-slate-950 hover:bg-slate-800 border border-slate-800 rounded-lg text-sm font-bold text-indigo-300 cursor-pointer"
                    >
                      Copy Response
                    </button>
                  </div>
              ...
              <div className="bg-slate-900 border border-slate-800 p-4 rounded-3xl space-y-4 shadow-2xl backdrop-blur-xl">
                <span className="text-sm text-purple-300 font-bold uppercase tracking-widest block">Practice Studio</span>
                <h3 className="text-sm font-black text-white uppercase">🎭 AI Objection Role-Play Simulator</h3>

                <div className="bg-slate-950 border border-slate-800 p-4 rounded-2xl">
                  <span className="text-amber-300 font-bold uppercase text-xs block">🗣️ Client Objection:</span>
                  <blockquote className="text-slate-200 text-xs italic font-sans mt-1">"{roleplayObjection}"</blockquote>
                </div>
                ...
                <form onSubmit={handleEvaluateRoleplay} className="space-y-2">
                  <textarea
                    ...
                    className="w-full bg-slate-950 border border-slate-800 p-3.5 text-white rounded-2xl outline-none focus:border-indigo-500 text-xs font-sans placeholder-slate-500 resize-none"
                  />
                  <button type="submit" className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-2xl uppercase tracking-wider transition-all cursor-pointer">
                    🎯 Evaluate Response
                  </button>
                </form>

                {roleplayFeedback && (
                  <div className="bg-indigo-950/40 border border-indigo-500/30 p-4 rounded-2xl space-y-1 animate-fadeIn">
                    <div className="flex justify-between font-bold text-xs">
                      <span className="text-indigo-300">Pitch AI Score:</span>
                      <span className="text-emerald-400 font-black">{roleplayFeedback.score}/100</span>
                    </div>
                    <p className="text-slate-200 text-xs font-sans">{roleplayFeedback.tip}</p>
                  </div>
                )}
              </div>
            </div>
      ...
      <AnimatePresence>
        {selectedLeadIds.length > 0 && (
          <motion.div
            ...
            exit={{ y: 50, opacity: 0 }}
            className="fixed bottom-[calc(max(20px,env(safe-area-inset-bottom))+60px)] left-1/2 -translate-x-1/2 z-[130] bg-slate-900 border border-slate-800 p-2.5 rounded-full shadow-2xl flex items-center gap-3 font-mono text-xs backdrop-blur-xl"
          >
            <span className="px-3 py-1 bg-indigo-600/30 text-indigo-200 font-bold rounded-full">
              {selectedLeadIds.length} Selected
            </span>
            <button onClick={handleBulkMarkWon} className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-full cursor-pointer flex items-center gap-1">
              <CheckCircle2 className="w-3.5 h-3.5" /> Mark Won
            </button>
            <button onClick={() => setSelectedLeadIds([])} className="p-1.5 text-slate-400 hover:text-white rounded-full">
              <X className="w-4 h-4" />
            </button>
          </motion.div>
        )}
      ...
      {activeWorkspaceLead && (
        <div className="fixed inset-0 z-[120] bg-slate-950 flex flex-col no-print h-[100dvh] w-full overflow-hidden">
          
          {/* 🟢 TOP CONTROL HEADER (CLEARING DYNAMIC ISLAND / NOTCH) */}
          <header className="bg-slate-900 border-b border-slate-800 px-3 pb-2.5 pt-[max(16px,env(safe-area-inset-top))] font-mono text-xs flex items-center justify-between gap-1.5 shrink-0 z-30 shadow-xl">
            <div className="flex items-center gap-1.5">
              <button
                onClick={closeWorkspaceMatrix}
                className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-white border border-slate-700 rounded-xl font-bold flex items-center gap-1 cursor-pointer transition-all active:scale-95 text-sm"
              >
                ✕ Exit
              </button>
            ...
            <span className="hidden sm:block text-xs font-semibold text-slate-300 truncate px-2">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <button
                ...
                className="px-2.5 py-1.5 bg-slate-800 hover:bg-slate-700 text-indigo-300 border border-slate-700 rounded-xl font-bold text-xs cursor-pointer transition-all"
              >
                📱 QR
              </button>
              <button
                onClick={() => { playAudioChime('click'); setIsQuoteModalOpen(true); }}
                className="px-2.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-xl text-xs uppercase cursor-pointer transition-all shadow-md"
              >
                🧮 Quote
              </button>
            </div>
          ...
          {/* 🎛️ FULL CONFIG DECK */}
          {!isConfigDeckCollapsed && (
            <div className="bg-slate-900 border-b border-slate-800 p-4 font-mono text-sm space-y-3 z-30 shadow-2xl shrink-0 animate-slideDown max-h-[50vh] overflow-y-auto">
              <div className="flex justify-between items-center border-b border-slate-800 pb-2">
                <span className="text-xs text-indigo-400 uppercase font-black tracking-widest flex items-center gap-1.5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-3">
                <div className="space-y-1">
                  <label className="text-slate-400 font-bold block text-xs">Business Name</label>
                  <input
                    type="text"
                    value={wsName}
                    onChange={(e) => setWsName(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-800 p-2 text-white rounded-xl font-sans text-xs outline-none focus:border-indigo-500"
                  />
                </div>

                <div className="space-y-1">
                  <label className="text-slate-400 font-bold block text-xs">Website Domain</label>
                  <input
                    type="text"
                    value={wsUrl}
                ...
                <div className="space-y-1">
                  <label className="text-slate-400 font-bold block text-xs">Monthly Missed Revenue ($)</label>
                  <input
                    type="number"
                    value={wsLoss}
                    onChange={(e) => setWsLoss(Number(e.target.value))}
                    className="w-full bg-slate-950 border border-slate-800 p-2 text-emerald-400 font-mono text-xs outline-none focus:border-indigo-500 font-bold"
                  />
                </div>

                <div className="space-y-1">
                  <label className="text-slate-400 font-bold block text-xs">Industry Sector</label>
                  <select
                    value={activeIndustry}
                    onChange={(e) => setActiveIndustry(e.target.value as Industry)}
                    className="w-full bg-slate-950 border border-slate-800 p-2 text-white rounded-xl font-sans text-xs outline-none focus:border-indigo-500 cursor-pointer"
                  >
      ...
      {showQrModal && activeWorkspaceLead && (
        <div className="fixed inset-0 z-[140] bg-black/90 backdrop-blur-md flex items-center justify-center p-4 animate-fadeIn no-print" onClick={() => setShowQrModal(false)}>
          <div className="bg-slate-900 border border-slate-800 p-4 rounded-3xl shadow-2xl flex flex-col items-center space-y-4 max-w-sm w-full text-center" onClick={(e) => e.stopPropagation()}>
            <span className="text-xs font-mono font-black uppercase text-indigo-400 tracking-widest">Live Audit Phone Transfer</span>
            <div className="bg-white p-4 rounded-2xl border border-slate-800"><QRCode value={window.location.origin + liveIframePreviewUrl} size={210} /></div>
            <p className="text-xs text-slate-300 font-mono font-bold">Scan to open <span className="text-white">{wsName}</span> on client device</p>
            <button onClick={() => setShowQrModal(false)} className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 text-white font-mono text-xs font-bold rounded-xl cursor-pointer min-h-[40px]">✕ Close QR Screen</button>
          </div>
        </div>
      )}
      ...
      {isQuoteModalOpen && (
        <div className="fixed inset-0 z-[140] flex items-center justify-center p-2 sm:p-4 bg-black/90 backdrop-blur-md animate-fadeIn font-mono text-xs h-[100dvh]">
          <div className="bg-slate-900 border border-slate-800 max-w-4xl w-full max-h-[92dvh] overflow-y-auto rounded-3xl p-2 shadow-2xl relative flex flex-col">
            <div className="flex justify-end p-2">
              <button type="button" onClick={() => setIsQuoteModalOpen(false)} className="w-8 h-8 flex items-center justify-center bg-slate-800 text-white rounded-full cursor-pointer text-sm font-bold">✕</button>
            </div>
