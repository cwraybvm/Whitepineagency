...
function KpiCardSkeleton() {
  return (
    <div className="bg-slate-100 dark:bg-slate-950/80 border border-emerald-900/10 dark:border-slate-800/80 p-4 rounded-2xl flex flex-col justify-between animate-pulse">
...
export default function AdminPage() {
  ...
  return (
    <div ref={pageRef} className="admin-console min-h-[100dvh] antialiased relative pb-36 select-none transition-colors pt-[calc(max(12px,env(safe-area-inset-top))+8px)] bg-[#F1F5F2] text-slate-800 dark:bg-[#0B0F15] dark:text-gray-200">
      ...
      <header className="max-w-7xl mx-auto px-3 sm:px-4 pt-2 sm:pt-4 no-print relative z-10 font-mono text-xs">
        <div className="bg-white/90 dark:bg-[#121824]/80 border border-emerald-900/10 dark:border-slate-800/80 border-t-white/80 dark:border-t-white/10 rounded-2xl sm:rounded-3xl p-3 sm:p-4 backdrop-blur-2xl shadow-2xl space-y-3 sm:space-y-4">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 sm:gap-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 w-full lg:w-auto">
              {isLoadingLeads ? (
                ...
                <>
                  <div className="gsap-card bg-slate-100 dark:bg-slate-950/80 border border-emerald-900/10 dark:border-slate-800/80 hover:border-slate-700 transition-colors p-4 rounded-2xl flex flex-col justify-between">
          ...
          {/* View switcher for this page's own content (Overview/Studio/Vault) --
              deliberately styled as an underlined tab strip, not a pill-button
              bar, so it doesn't read as a second primary nav next to AdminNav's
              "Pipeline" bottom-nav/sidebar link. */}
          <div className="flex flex-wrap justify-between items-center pt-3 border-t border-emerald-900/10 dark:border-slate-800/80 gap-3">
            <div className="flex w-full sm:w-auto justify-between sm:justify-start text-xs border-b border-emerald-900/10 dark:border-slate-800/80">
              <button
                onClick={() => { playAudioChime('click'); setActiveTab('pipeline'); }}
                className={`flex-1 sm:flex-none px-3.5 py-2 font-semibold uppercase transition-all cursor-pointer flex items-center gap-1.5 border-b-2 -mb-px ${
                  activeTab === 'pipeline' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'
                }`}
              >
                <Layers className="w-3.5 h-3.5" /> Overview <kbd className="hidden lg:inline text-[10px] normal-case bg-black/10 dark:bg-white/10 border border-black/10 dark:border-white/10 border-b-2 border-b-black/25 dark:border-b-white/25 rounded px-1.5 py-0.5 shadow-sm">1</kbd>
              </button>
              <button
                onClick={() => { playAudioChime('click'); setActiveTab('pitch'); }}
                className={`flex-1 sm:flex-none px-3.5 py-2 font-semibold uppercase transition-all cursor-pointer flex items-center gap-1.5 border-b-2 -mb-px ${
                  activeTab === 'pitch' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'
                }`}
              >
              ...
              <button
                ...
                className={`flex-1 sm:flex-none px-3.5 py-2 font-semibold uppercase transition-all cursor-pointer flex items-center gap-1.5 border-b-2 -mb-px ${
                  activeTab === 'vault' ? 'border-emerald-500 text-emerald-600 dark:text-emerald-400' : 'border-transparent text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-white'
                }`}
              >
