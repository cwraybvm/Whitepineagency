...
function buildModules(metrics: HubMetrics): Module[] {
  return [
    {
      key: 'admin',
      hotkey: '1',
      ...
      href: '/admin',
      badge: 'INTERNAL OPS',
      status: metrics.unreadSignals !== null ? `${metrics.unreadSignals} Unread Signals` : 'System Nominal',
      accent: '#0EA5E9',
    ...
    },
    {
      ...
      href: '/crm',
      badge: 'SALES & LEADS',
      status: metrics.activeLeads !== null ? `Active — ${metrics.activeLeads} Leads` : 'Active Leads',
      accent: '#10B981',
    ...
    },
    {
      ...
      badge: 'CLIENT FACING',
      status: metrics.liveOrgs !== null ? `${metrics.liveOrgs} Orgs Live` : 'Client Portal',
      accent: '#3B82F6',
...
export default function HubLaunchpad({ metrics }: { metrics: HubMetrics }) {
  const router = useRouter();
  const modules = useMemo(() => buildModules(metrics), [metrics]);
  const hotkeyMap = useMemo(() => new Map(modules.map((m) => [m.hotkey, m.href])), [modules]);
